...
def view_sign_in_layout_creation(page: ft.Page):
    ...
    def validate_password(password:str):
            if users[login_text_field.value] == password:
                print(users)
                return True


    def after_sign_in_validation(e):
        # global identified_user
        # identified_user = login_text_field.value
    ...
    def validation_password_login(control:ft.control, user_input_str:str):
        if control == login_text_field:
            ...
            if login_text_field.data == 1:
                valid_val_password = validate_password(user_input_str)
    ...
    sign_in_button = ft.ElevatedButton('SIGN IN', on_click=after_sign_in_validation)
